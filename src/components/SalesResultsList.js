...
function SalesResultsList() {
  ...
  const fetchSalesResults = useCallback(async () => {
    try {
      ...
      console.log('✅ 成約案件一覧取得成功:', filteredResults.length, '件 (除外:', results.length - filteredResults.length, '件)');
  ...
  const getSummary = () => {
    // 管理者画面では「他社案件」を除外
    const validResults = salesResults.filter(deal => deal.proposalMenu !== '他社案件');
    const totalAmount = validResults.reduce((sum, deal) => sum + (deal.receivedOrderAmount || 0), 0);
    ...
    const thisMonthResults = validResults.filter(deal => {
